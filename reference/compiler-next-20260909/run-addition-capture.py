...
def captured_check(root,label,include_callees=False):
    ...
    entry=root/NEXT/('capture-addition-checked.factor' if include_callees else 'capture-checked.factor')
    cmd=['taskset','-c','2',str(root/'factor'),'-resource-path='+str(root),
         '-i='+str(root/NEXT/'prepared.image'),'-no-user-init',str(entry),
    ...
    if include_callees: expected={'math:+', ':fixnum=>+'}
...
if a.phase=='capture-callees':
    captured_check(BASE,'corrected-baseline-addition-check',True)
    captured_check(a.candidate,'corrected-candidate-addition-check',True)
    raise SystemExit(0)
