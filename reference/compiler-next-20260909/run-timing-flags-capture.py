...
def captured_check(root,label,include_callees=False):
    ...
    entry=root/NEXT/('capture-timing-flags.factor' if include_callees else 'capture-checked.factor')
    cmd=['taskset','-c','2',str(root/'factor'),'-resource-path='+str(root),
         '-i='+str(root/NEXT/'prepared.image'),'-no-user-init',str(entry),
         'backtracking','timing','0','on','on',source]
    ...
    if include_callees: expected.update({'math:+', ':fixnum=>+', 'allocator-runtime-comparison:integer-pressure'})
    ...
    status=dict(ok=ok,exit_code=child.returncode,source_commit=source,command=cmd,
        diagnostic_only=True,final_value_verifier=False,seconds=time.monotonic()-start,
        ...
        script_sha256={n:hashlib.sha256((root/NEXT/n).read_bytes()).hexdigest()
                       for n in [entry.name,'capture-workload-code.factor','capture-installed.factor','capture-timing-flags-body.factor']})
...
if a.phase=='capture-callees':
    captured_check(BASE,'corrected-baseline-timing-flags-capture',True)
    captured_check(a.candidate,'corrected-candidate-timing-flags-capture',True)
    raise SystemExit(0)
