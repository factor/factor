...
out=P/'corrected-native/final-a7/addition-capture'; cap={}
for v,root in [('baseline','/home/erg/factor-compiler-next-greedy-baseline-20260909'),('candidate','/home/erg/factor-compiler-next-final-c6948-20260909')]:
 n='corrected-'+v+'-addition-check'; f=c.fetch(root,c.NXT,[n+e for e in ['.log','.jsonl','.status.json']]+['capture-addition-checked.factor','capture-workload-code.factor'],out/v)
 c.fetch(root,c.NXT,['captured-addition-'+v+'.bin','captured-addition-'+v+'.asm'],out/v)
...
assert set(cap['baseline'])==set(cap['candidate']) and len(cap['baseline'])==2
result={}
