...
:: sample ( word trial -- )
    word word-id batch-counts at [ ] [ word witness-batch-count ] if* :> iterations
...
:: main ( -- )
    ...
    command-line get 7 swap nth :> feature
    command-line get 8 swap nth "on" = :> enabled?
    feature enabled? configure-features :> features
    ...
    H{ { "rematerialize_constants" rematerialize }
       { "backtracking_loop_spills" loop-spills } { "gvn" gvn } { "features" features } } "options" pick set-at
    ...
    benchmark-metric-workloads [| target |
        ...
        target word-id :> id
        H{ { "kind" "code" } { "metric-word" id } { "report" report } } emit
    ] each
    benchmark-runtime-workloads [| word | word -1 sample ] each
    checked [ ] [ command-line get third string>number [| trial | benchmark-runtime-workloads [| word | word trial sample ] each ] each-integer ] if ;
main
