...
for feature,enabled,label in [('loops',False,a.prefix+'-baseline')]+[(f,True,a.prefix+'-'+f+'-on') for f in features]:
 ...
 expected=dict(gvn=False,rematerialize_constants=False,backtracking_loop_spills=False,features={f:f==feature and enabled for f in features});assert scope['options']==expected
...
result={'ok':True,'source_commit':identities[0][0],'image_sha256':identities[0][1],'vm_sha256':identities[0][2],'scope_count':len(scopes[0]),'runtime_cases':30,'metric_targets':16,'checks':5,'outputs':outputs[0],'metric_words':metrics[0],'record_sha256':hashes}
(a.directory/'checked-validation.json').write_text(json.dumps(result,indent=2)+'\n');print('FIVE CONFIGURATIONS MATCH',len(scopes[0]),30,16)
