...
OUT=ROOT/'reference/compiler-next3-20260909'
...
p.add_argument('--feature',choices=['loops','representations','memory','slp'],required=True)
...
for r in range(1 if a.mode=='check' else a.rounds):
    ...
    for allocator in order:
        ...
        ok=ok and scope.get('options')=={'rematerialize_constants':a.rematerialize=='on','backtracking_loop_spills':a.loop_spills=='on','gvn':a.gvn=='on','features':{f:(f==a.feature and a.enabled=='on') for f in ['loops','representations','memory','slp']}} and scope.get('checked')==(a.mode=='check')
