#!/usr/bin/env python3
"""Validate combined feature records, retain all cases, and separate corpus/witnesses."""
...
def read(label,feature,enabled,mode,ordinal):
 ...
 assert scope['options']==dict(gvn=False,rematerialize_constants=False,backtracking_loop_spills=False,features={f:enabled for f in features})
...
checked={s:read(a.prefix+'-'+s,'all',s=='on','check',1) for s in ['off','on']}
old_equal=None
...
for f in ['all']:
 runs={s:[read(a.prefix+'-'+s,'all',s=='on','timing',i) for i in [1,2]] for s in ['off','on']};observations[f]=runs
 equal={};checked_equal={}
 for s in ['off','on']:
...
assert measured==360
result={'accepted':True,'source_commit':identity[0],'image_sha256':identity[1],'vm_sha256':identity[2],'checked_processes':2,'timing_processes':4,'protocol':'combined','temporal_order':['off','on','on','off'],'measured_batches':measured,'scope_count':len(scope_words),'language_outputs_match':True,'corpus_cases':corpus,'witness_cases':witnesses,'metric_ids':metric_ids,'default_off_first12_match_old':old_equal,'all_off_groups_match_checked_baseline':True,'features':results,'observations':observations}
