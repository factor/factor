...
OUT=ROOT/'reference/compiler-combined-20260909'
...
p.add_argument('--allocator',choices=ALLOCATORS,default='linear-scan')
p.add_argument('--rematerialize',choices=['on','off'],default='off')
p.add_argument('--feature',choices=['all'],default='all')
p.add_argument('--enabled',choices=['on','off'],required=True)
...
for r in range(1 if a.mode=='check' else a.rounds):
    ...
    for allocator in order:
        ...
        if a.mode=='check' and (ROOT/'basis/compiler/cfg/register-allocation/verifier/verifier.factor').exists():
            ...
            setup+=' >>\n"reference/compiler-combined-20260909/timing.factor" run-file\n'
            entry=OUT/'checked-launch.factor'
            entry.write_text(setup)
        cmd=[str(ROOT/'factor'),'-resource-path='+str(ROOT),'-i='+str(Path(a.image).resolve()),'-no-user-init',str(entry),allocator,a.mode,str(a.samples),a.rematerialize,a.loop_spills,revision,a.gvn,a.feature,a.enabled]
