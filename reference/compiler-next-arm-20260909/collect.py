...
REVISIONS={'baseline':'c1f7e4d34cd604bbdf22576b237406b39758896d','candidate':'eccfc0accc5c513b44701f2e59d4a9bafb7515f0'}
...
audit={'scope':'Targeted ARM LS/backtracking timing; all-four allocator correctness, not all-four timing.', 'sources':{},'runs':{},'pending':[],'comparisons':{}}
...
for label,root in ROOTS.items():
 ...
 prep=json.loads((root/NEXT/'arm-next-prepare.status.json').read_text())
 assert prep['status']==0 and prep['source_commit']==REVISIONS[label]
 assert sha(root/NEXT/'prepared.image')==prep['prepared_image_sha256']
 ...
 for src,dest in [(root/REL/'source-expected.json',OUT/label/'source-expected.json.gz'),(root/REL/'source-manifest.json',OUT/label/'source-manifest.json.gz'),(root/NEXT/'arm-next-prepare.status.json',OUT/label/'prepare.status.json'),(root/NEXT/'arm-next-prepare.log',OUT/label/'prepare.log.gz')]:archive(src,dest)
 names=[f'arm-next-{label}-timing-{a}-{r}' for a in ['linear-scan','backtracking'] for r in [1,2]]
 if label=='candidate':names += [f'combined-check-{a}-1' for a in ['linear-scan','greedy','backtracking','chordal']]
 for name in names:
...
if scopes:
 ...
 if b is not None and c is not None:
  ...
  expected_added=['compiler.cfg.register-allocation.backtracking:backtracking-edge-entry-reloads','compiler.cfg.register-allocation.backtracking:edge-entry-reload?','compiler.cfg.register-allocation.backtracking:reify-entry-transports','compiler.cfg.register-allocation.chordal:preferred-free-color']
  assert not removed and added==expected_added,(removed,added)
...
for allocator in ['linear-scan','backtracking']:
 rounds=[]
 for ordinal in [1,2]:
  b=f'arm-next-baseline-timing-{allocator}-{ordinal}';c=f'arm-next-candidate-timing-{allocator}-{ordinal}'
 ...
 if len(rounds)==2:
  ...
  for label in ROOTS:
   names=[f'arm-next-{label}-timing-{allocator}-{r}' for r in [1,2]]
...
for allocator in ['linear-scan','greedy','backtracking','chordal']:
 p=Path('/tmp/compiler-next-combined-'+allocator)
 for fname in ['status.json','output.log']:archive(p/fname,OUT/'full-compiler'/allocator/(fname+('.gz' if fname.endswith('.log') else '')))
 if (p/'status.json').exists():assert json.loads((p/'status.json').read_text())['exit_code']==0
for label,dirname in [('candidate','compiler-next-default-bootstrap'),('baseline','compiler-next-baseline-bootstrap')]:
 p=Path('/tmp')/dirname
...
archive(Path('/tmp/compiler-next-arm-next.py'),OUT/'queue-driver.py')
