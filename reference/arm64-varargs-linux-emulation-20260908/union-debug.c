...
UNION_EXPORT int au_c_controls(void) {
    ...
    if (au_take_one(one,10)!=11.25 || au_take_two(two,10)!=16.25 || au_take_four(four,10)!=40 || au_take_nested(nested,10)!=16.25 || au_take_mixed(mixed,10)!=13.75) { printf("control group 1 failed\n"); return 0; }
    if (au_return_one(1.25f).a!=1.25f || au_return_two(1.25f,2.5f).pair.b!=2.5f || au_return_four(1,2,3,4).quad.d!=4) { printf("control group 2 failed\n"); return 0; }
    if (au_call_one(au_take_one)!=11.25 || au_call_two(au_take_two)!=16.25 || au_call_four(au_take_four)!=40) { printf("control group 3 failed\n"); return 0; }
    if (au_variadic(7,two,10.0)!=23.25 || au_call_variadic(au_variadic)!=23.25) { printf("control group 4 failed\n"); return 0; }
    if (au_take_padded(padded,10)!=16.25 || au_take_padded_union(padded_union,10)!=16.25 || au_return_padded(1.25f,2.5f).b!=2.5f || au_call_padded(au_take_padded)!=16.25) { printf("control group 5 failed\n"); return 0; }
    if (au_take_spill(1,2,3,4,5,6,two,10)!=37.25 || au_call_spill(au_take_spill)!=37.25) { printf("control group 6 failed\n"); return 0; }
#if defined(__clang__) || defined(__GNUC__)
    union au_vector vector; vector.a=(au_fvec){1,2,3,4};
    printf("vector two %.17g; vector variadic %.17g\n",au_take_vector_two(au_return_vector_two(),10),au_call_vector_variadic(au_vector_variadic_control));
    if (au_take_vector_two(au_return_vector_two(),10)!=46 || au_call_vector_variadic(au_vector_variadic_control)!=53) { printf("control group 7 failed\n"); return 0; }
    if (au_take_vector(vector,10)!=40 || au_return_vector(1,2,3,4).a[3]!=4 || au_call_vector(au_take_vector)!=40) { printf("control group 8 failed\n"); return 0; }
#endif
#if defined(__aarch64__) && defined(__clang__) && __clang_major__ >= 18
    if (!au_small_controls()) { printf("control group 9 failed\n"); return 0; }
#endif
    return 1;
}
