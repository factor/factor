...
for label in ('baseline', 'candidate'):
    ...
    input_label = getattr(a,label+'_label')
    names = [input_label+n[len(label):] for n in names]
    canonical_names += ['source-expected.json', 'source-manifest.json']
    ...
    files.update({canonical:files[actual] for canonical,actual in zip(canonical_names,names)})
...
(a.output / 'collection.json').write_text(json.dumps(dict(accepted=True, remote_host=a.remote_host,
    main_measured_batches=1248, attribution_measured_batches=0, measured_batches=1248,
    checked_batches=208, sources=provenance), indent=2) + '\n')
print('Collected 16 main + 0 attribution + 8 checked runs; 1,248 measured batches and 208 checked outputs')
