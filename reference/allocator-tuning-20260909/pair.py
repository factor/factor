...
for r in range(a.rounds if a.mode=='timing' else 1):
 ...
 for allocator in order:
  for label,root in roots:
   driver=root.resolve()/'reference/allocator-tuning-20260909/drive.py'
   ...
   selected_image=getattr(a,label+'_image')
   if selected_image:cmd+=['--image',str((root/selected_image).resolve())]
   cmd[2]=getattr(a,label+'_label')
