...
syn keyword factorKeyword global +@ set-namestack change-global with-variable on toggle set bind dec initialize namestack get get-global change init-namespaces with-scope off set-global namespace inc counter make-assoc with-global
...
syn keyword factorKeyword with-return restarts return-continuation with-datastack recover in-callback? rethrow-restarts throw-continue <restart> ifcc original-error error-in-thread cleanup restart? ignore-errors compute-restarts attempt-all-error error-thread continue <continuation> attempt-all-error? condition? <condition> throw-restarts error restart thread-error-hook continue-with continuation rethrow callcc1 error-continuation callcc0 condition callback-error-hook continuation? attempt-all return
