USING: help.markup help.syntax ui.commands kernel models ;
IN: ui.gadgets.tables

HELP: <table>
{ $values { "rows" model } { "renderer" "a row renderer" } { "table" table } }
{ $description "Creates a table displaying the rows in a model using the given renderer. Wrap a sequence of rows with " { $link <model> } " before passing it to this word." }
{ $errors "Throws an error if rows is not a model." } ;
...
$nl
"Tables display a model as a series of rows. The model's value must be a sequence, and a " { $emphasis "renderer" } " creates a sequence of columns for each row. Tables are built from and inherit all features of " { $link "ui.gadgets.line-support" } "."
