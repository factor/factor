...
{ $description "Creates an editor gadget with a blank model. Whenever a value is entered into the editor and Return or keypad Enter pressed, the value is pushed on the stack as a string and the specified quotation is called. Note that the quotation cannot update the value in the field." } ;
