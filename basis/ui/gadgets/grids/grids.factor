...
math.rectangles math.vectors opengl sequences strings.tables
...
: iterate-cell-dims ( cells quot -- seq )
    '[ [ pref-dim>> @ ] [ max ] map-reduce gl-ceiling ] map ; inline
...
: accumulate-cell-dims ( seq gap -- n ns )
    dup '[ + _ + gl-ceiling ] accumulate ;

: accumulate-cell-xs ( grid-layout -- x xs )
    [ column-widths>> ] [ gap>> first ] bi accumulate-cell-dims ;

: accumulate-cell-ys ( grid-layout -- y ys )
    [ row-heights>> ] [ gap>> second ] bi accumulate-cell-dims ;
...
: adjust-for-baseline ( row-locs row-cells -- row-locs' )
    align-baselines [ 0 swap 2array v+ [ gl-round ] map ] 2map ;
