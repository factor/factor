...
"The diagnostic association reports assignments, evictions, splits, region-splits, clobber-splits, and memory-only-clobbers when their counts are nonzero. Forced ABI operands have reserved spill slots; a fragment used only by a clobber is represented by its slot. Measurements and executable pressure tests accompany the implementation."
