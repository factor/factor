...
extends through the final phase of the block. `phase-block-from` is the first
instruction's number, so the predecessor's late phase is strictly before
its layout successor's entry. Sharing the old `first-number - 1` boundary
would create false interference between values on different CFG paths.
...
lifecycle. Entry reloads are emitted inside the successor block; incoming
edge maps and phi destinations name their pre-reload homes, ensuring every
predecessor initializes those homes before the reload. This prevents an old fragment from deleting its replacement's
