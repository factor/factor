...
{ $description "Formats and prints a profile report generated by " { $link top-down } ", " { $link top-down-max-depth } ", " { $link cross-section } ", or " { $link flat } ". Threads and sibling calls are sorted by decreasing running time. The time % column shows each row's share of the total sampled time across all threads. The GC, JIT, FFI and FT percentages are relative to that row's running time. Inclusive call times overlap, so percentages should not be summed across callstack levels." }
{ $see-also profile-minimum-percent } ;

HELP: profile-minimum-percent
{ $var-description "Minimum percentage of the report's total sampled time a row must account for to be printed by " { $link profile. } ". Defaults to zero, showing all rows. Filtering also hides descendants of excluded rows and does not change the percentage denominator." }
{ $examples { $code "10 profile-minimum-percent [ top-down profile. ] with-variable" } } ;
