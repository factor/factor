...
! Native SDK vtable order, excluding inherited IUnknown slots.
{ { "Next" "Skip" "Reset" "Clone" } } [
    IEnumFORMATETC find-com-interface-definition functions>> [ name>> ] map
] unit-test

{ { "QueryContinueDrag" "GiveFeedback" } } [
    IDropSource find-com-interface-definition functions>> [ name>> ] map
] unit-test

{ { "CreateStream" "OpenStream" "CreateStorage" "OpenStorage"
    "CopyTo" "MoveElementTo" "Commit" "Revert" "EnumElements"
    "DestroyElement" "RenameElement" "SetElementTimes" "SetClass"
    "SetStateBits" "Stat" } } [
    IStorage find-com-interface-definition functions>> [ name>> ] map
] unit-test
