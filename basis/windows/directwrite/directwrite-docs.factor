...
{ $notes "The size slot contains bitmap bounds including glyph overhang. The metrics slot contains logical text dimensions; the origin slot is the translation required when drawing into the bitmap. Tab intervals are uniform, not arbitrary tab-stop lists. Unavailable font families resolve to Segoe UI for both shaping and font metrics."
    $nl
    "Long left-to-right printable ASCII lines retain an index of native shaped glyphs. Viewport rendering visits only intersecting glyph blocks, and width is calculated from their advances to avoid single-precision accumulation errors. Complex scripts continue to use native layout rendering. Equal output strings share a layout; repeated lookups of the same string object avoid comparing the whole string again."
    $nl
