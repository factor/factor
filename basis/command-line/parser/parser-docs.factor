...
{ $class-description "An option that can be specified on the command-line. It has the following slots:"
    { $slots
        ...
        { "aliases" { "Additional names for an optional argument, such as " { $snippet "{ \"-H\" \"--hostname\" }" } ". Aliases share the canonical option's variable, default, validation and required status. Exact names and aliases take precedence over abbreviations and " { $snippet "no-" } " negation, and aliases are included in help output. The names and aliases of optional arguments, including the default " { $snippet "--help" } ", must be unique." } }
