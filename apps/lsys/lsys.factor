...
USING: kernel namespaces math vectors hashtables sequences threads gadgets
       math-contrib
       vars slate opengl rewrite-closures handler
       lsys.opengl lsys.turtle lsys.camera ;
...
! : init-slate ( -- ) [ display ] closed-quot <camera-slate> >slate ;

VAR: slate

: init-slate ( -- ) [ display ] closed-quot <slate> >slate ;

! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

! For compatibility. The code below assumes the old style slate.
...
T{ key-down f f "LEFT" }
[ drop [ 5 turn-left ] camera> with-turtle .slate ] closed-quot
put-hash

T{ key-down f f "RIGHT" }
[ drop [ 5 turn-right ] camera> with-turtle .slate ] closed-quot
put-hash

T{ key-down f f "UP" }
[ drop [ 5 pitch-down ] camera> with-turtle .slate ] closed-quot
put-hash

T{ key-down f f "DOWN" }
[ drop [ 5 pitch-up ] camera> with-turtle .slate ] closed-quot
put-hash


T{ key-down f f "a" }
[ drop [ 1 step-turtle ] camera> with-turtle .slate ] closed-quot
put-hash
...
T{ key-down f f "q" }
[ drop [ 5 roll-left ] camera> with-turtle .slate ] closed-quot
put-hash

T{ key-down f f "w" }
[ drop [ 5 roll-right ] camera> with-turtle .slate ] closed-quot
put-hash


T{ key-down f { A+ } "LEFT" }
[ drop [ 1 strafe-left ] camera> with-turtle .slate ] closed-quot
put-hash

T{ key-down f { A+ } "RIGHT" }
[ drop [ 1 strafe-right ] camera> with-turtle .slate ] closed-quot
put-hash

T{ key-down f { A+ } "UP" }
[ drop [ 1 strafe-up ] camera> with-turtle .slate ] closed-quot
put-hash

T{ key-down f { A+ } "DOWN" }
[ drop [ 1 strafe-down ] camera> with-turtle .slate ] closed-quot
put-hash
...
slate> over set-gadget-delegate "Lindenmayer" open-window ;
