...
USING: kernel namespaces threads math sequences
       opengl gadgets gadgets-theme gadgets-labels gadgets-buttons
       vars rewrite-closures opengl.colors opengl.lib gadgets.lib slate
...
USING: hashtables namespaces.lib handler ;
...
} make* make-pile 1 over set-pack-fill "L-system control" open-window ;
...
} make* make-pile 1 over set-pack-fill "L-system models" open-window ;
...
} make* make-pile 1 over set-pack-fill "L-system scenes" open-window ;

: lsys-window* ( -- ) lsys-controller lsys-viewer ;