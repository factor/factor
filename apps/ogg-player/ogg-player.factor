...
!
! TODO: 
! - Change usage of AL_FORMAT_STEREO16 to MONO16 or STEREO16 
...
!
USING: kernel errors alien ogg vorbis theora io byte-arrays sequences libc shuffle 
       openal math namespaces threads shuffle gadgets opengl arrays ;
IN: ogg-player
...
: append-new-audio-buffer ( player -- player )
    ...
    [ player-buffers second AL_FORMAT_STEREO16 ] keep
...
: fill-processed-audio-buffer ( player n -- player )
    ...
    >r dup player-source r> pick player-buffer-indexes
    [ alSourceUnqueueBuffers check-error ] keep 
    *uint dup >r AL_FORMAT_STEREO16 rot 
    [ player-audio-buffer dup length  ] keep
    [ player-vi vorbis_info-rate alBufferData check-error ]  keep 
    [ player-source 1 ] keep
...
: start-audio ( player -- player bool )
    [ player-buffers first AL_FORMAT_STEREO16 ] keep
    [ player-audio-buffer dup length ] keep
    [ player-vi vorbis_info-rate alBufferData check-error ]  keep 
    [ player-source 1 ] keep
...
: get-more-header-data ( player -- player )
    buffer-data [ 
        "End of file reached while searching for codec headers" throw  
    ] when ;
...
: decode ( player -- player )
    ...
    decode-audio
    decode-video
    dup player-audio-full? over player-video-ready? not or [
