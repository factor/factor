...
{ $examples
    { $example
        "USING: io io.styles markdown.streams prettyprint ;"
        "[ \"Hello\" { { font-style bold } } format nl ] with-markdown-writer ."
        "\"**Hello**\\n\\n\""
    }
} ;

ARTICLE: "markdown.streams" "Markdown streams"
"The " { $vocab-link "markdown.streams" } " vocabulary provides a stream which implements " { $link "io.styles" } " by constructing Markdown markup in the form of " { $link string } " types."
{ $subsections
    markdown-writer
    <markdown-writer>
