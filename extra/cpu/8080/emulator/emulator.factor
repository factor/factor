...
: patterns ( -- hashtable )
    ...
    H{
        ...
        { "JP-F|FF,NN" [ [ $1 ] 1check [ [ next-word ] keep [ pc<< ] keep [ cycles>> ] 1check 5 + swap cycles<< ] [ [ pc>> 2 + ] keep pc<< ] if ] }
        { "JP-(RR)" [ [ $1 ] keep pc<< ] }
        { "CALL-NN" [ (emulate-CALL) ] }
        { "CALL-F|FF,NN" [ [ $1 ] 1check [ 7 over inc-cycles (emulate-CALL) ] [ [ pc>> 2 + ] keep pc<< ] if ] }
