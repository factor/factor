...
prettyprint sequences sets sorting splitting strings summary
vocabs words ;
FROM: namespaces => set ;
...
: write-object-seq ( object-seq -- )
    [
        dup array? [
            dup ?first array?
            [ [ first2 write-object " " write ] each ]
...
: (present-coverage) ( coverage-report -- )
    ...
    dup 100%-coverage?>> [ drop "full help coverage" over push ]
    [ [ (assemble-empty-examples) ] [ (assemble-omitted-sections) ] bi ] if
    "\n" over push
    write-object-seq ; inline
...
M: string <word-help-coverage>
    loaded-vocab-names resolve-name-in call-next-method ; inline
