...
: 1check-decimals ( obj1 obj2 -- D1 D2 )
...
M: decimal before?
    1check-decimals scale-decimals [ mantissa>> ] bi@ < ;

: D-abs ( D -- D' )
    [ mantissa>> abs ] [ exponent>> ] bi <decimal> ;

: D+ ( D1 D2 -- D3 )
    1check-decimals scale-mantissas [ + ] dip <decimal> ;

: D- ( D1 D2 -- D3 )
    1check-decimals scale-mantissas [ - ] dip <decimal> ;

: D* ( D1 D2 -- D3 )
    1check-decimals [ >decimal< ] bi@ swapd + [ * ] dip <decimal> ;

:: D/ ( D1 D2 a -- D3 )
    D1 D2 1check-decimals [ >decimal< ] bi@ :> ( m1 e1 m2 e2 )
    m1 a 10^ *
    m2 /i
