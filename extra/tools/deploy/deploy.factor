...
: strip-startup-hooks ( -- )
    ...
    "command-line" startup-hooks get delete-at ;
...
: strip-word-defs ( words -- )
    "Stripping interpreter definitions from compiled words" show
...
: strip-environment ( retain-globals -- )
    ...
    strip-globals? get [
        global strip-assoc 4 setenv
    ] [ drop ] if
    { 2 5 9 15 18 } [ f swap setenv ] each ;
...
: set-boot-quot* ( word -- )
    [
        ...
        ,
        "io.backend" startup-hooks get at [ \ flush , ] when
