...
USING: alien arrays byte-arrays errors gadgets gadgets-panes
graphics-gadget io kernel kernel-internals libc math
...
: parse-bitmap-header ( bitmap -- )
    ...
    4 read le> <int> *int over set-bitmap-height
...
: default ( obj/f default -- obj/default )
    #! if obj/f is f, use the default
    dupd ? ;
...
: save-bitmap ( bitmap path -- )
    <file-writer> [
        ...
        dup bitmap-planes 1 default 2 >le write
        dup bitmap-bit-count 24 default 2 >le write
        dup bitmap-compression 0 default 4 >le write
...
: test-bitmap ( -- )
    "libs/graphics/bmps/gaim-header.bmp" ! 57x150
