...
void inline_cache_jit::emit_inline_cache(fixnum index, cell generic_word_,
  ...
  // Leave the PIC before entering the miss handler. The shared resume word
  // owns the live frame, so replacing the call target may reclaim this PIC.
...
cell factor_vm::inline_cache_miss(cell return_address_) {
  ...
  if (pic_size < max_pic_size) {
    ...
    if (to_boolean(added)) {
      ...
      code_block* compiled = jit.to_code_block(CODE_BLOCK_PIC, JIT_FRAME_SIZE);
      initialize_code_block(compiled);
      xt = compiled->entry_point();
    }
  }
  ...
  if (return_address.valid) {
    ...
    // instead of leaving dead PICs around until the next GC. The miss ran
    // off-stack through inline-cache-miss-resume, so the PIC is not live.
