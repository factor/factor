...
struct slot_checker {
  ...
  void check_write_barrier(cell* slot_ptr, generation target, cell mask) {
    ...
    cell slot_card_pointer = parent->cards_offset + (reinterpret_cast<cell>(slot_ptr) >> card_bits);
    char slot_card_value = *reinterpret_cast<char*>(slot_card_pointer);
    if ((slot_card_value & mask) != mask) {
      ...
      std::cout << "object: 0x" << std::hex << reinterpret_cast<cell>(obj) << std::dec << std::endl;
      std::cout << "object type: " << obj->type() << std::endl;
      std::cout << "slot pointer: 0x" << std::hex << reinterpret_cast<cell>(slot_ptr) << std::dec << std::endl;
