...
cell factor_vm::second_from_top_stack_frame(context* stack_ctx) {
  cell frame_top = stack_ctx->callstack_top;
  for (cell i = 0; i < 2; ++i) {
    cell pred = code->frame_predecessor(frame_top);
    if (pred >= stack_ctx->callstack_bottom)
      return frame_top;
    frame_top = pred;
  }
  return frame_top;
...
cell factor_vm::capture_callstack(context* stack_ctx) {
  cell top = second_from_top_stack_frame(stack_ctx);
  cell bottom = stack_ctx->callstack_bottom;
