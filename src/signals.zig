...
// macOS/arm64 delivers FP traps as SIGILL; the FP cause lives in FPSR, or (when
// FPSR is clear) in the ESR exception-class field of the saved mcontext.
...
fn synchronousSignalHandler(sig: std.posix.SIG, _: *const SiginfoType, ucontext_ptr: ?*anyopaque) callconv(.c) void {
    ...
    // macOS/arm64 delivers FP traps as SIGILL. If we can decode FP trap bits
    // from the saved machine state, route it to the FP handler instead of
    // treating it as an illegal instruction.
