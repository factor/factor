...
TUPLE: source-file-error error asset path line-number ;

M: source-file-error error-file [ error>> error-file ] [ path>> ] bi or ;
M: source-file-error error-line [ error>> error-line ] [ line-number>> ] bi or ;
...
: new-source-file-error ( error asset class -- source-file-error )
    new
        swap
        [ >>asset ]
        [ where [ first2 [ >>path ] [ >>line-number ] bi* ] when* ] bi
        swap >>error ; inline

: sort-errors ( errors -- alist )
    [ [ line-number>> 0 or ] sort-with ] { } assoc-map-as sort-keys ;
