...
words classes definitions kernel alien combinators ;
IN: generic
...
"Neither " { $link general-t } " nor " { $link c-ptr } " are subclasses of each other, yet their intersection is the non-empty " { $link POSTPONE: f } " class. So the generic word system will place " { $link object } " first in the method order, however either " { $link general-t } " or " { $link c-ptr } " may come next, and it is pretty much a random choice that depends on hashing:"
