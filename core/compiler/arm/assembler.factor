...
USING: arrays generator generic kernel sequences words
namespaces math ;
IN: assembler-arm
...
: CC> ( -- n )
    ...
    cond-code [ f ] change [ BIN: 1110 ] unless* ;
...
G: (B) ( signed-imm-24 l -- ) 1 standard-combination ;

M: integer (B) { 24 { 1 25 } { 0 26 } { 1 27 } 0 } insn ;
M: word (B) 0 swap (B) rc-relative-arm-3 rel-word ;
M: label (B) 0 swap (B) rc-relative-arm-3 rel-label ;

: B 0 (B) ; : BL 1 (B) ;

G: (BX) ( Rm l -- ) 1 standard-combination ;
...
M: label (BX) 0 swap (BX) rc-relative-arm-3 rel-label ;
...
G: shift-imm/reg ( shift-imm/Rs Rm shift -- n )
...
GENERIC: addressing-mode ( addressing-mode -- n )

TUPLE: addressing p u w ;
C: addressing ( delegate p u w -- addressing )
    [ set-addressing-w ] keep
    [ set-addressing-u ] keep
    [ set-addressing-p ] keep
    [ set-delegate ] keep ;

M: addressing addressing-mode
    [ addressing-p ] keep
    [ addressing-u ] keep
    [ addressing-w ] keep
    delegate addressing-mode
    { 0 21 23 24 } bitfield ;

M: integer addressing-mode ;

M: object addressing-mode shifter-op { { 1 25 } 0 } bitfield ;
