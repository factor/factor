...
! This used to hang
[ { 0 1 } ] [
    [ [ [ drop 3 ] swap call ] dup call ] infer short-effect
] unit-test

[ t ] [
    [ [ [ dup call ] dup call ] infer short-effect ] catch
    inference-error?
] unit-test
...
[ V{ x } ] [ [ [ x get ] H{ } make-assoc ] infer drop inferred-vars-reads ] unit-test
[ V{ } ] [ [ [ x set ] H{ } make-assoc ] infer drop inferred-vars-writes ] unit-test
...
! be caught
: foo ( a b -- c ) over [ foo ] when ; inline
: bar foo ;

[ [ bar ] infer ] unit-test-fails
