...
T{ mismatched T{ parsing-error f 1 8 } T{ name f "" "x" f } T{ name f "" "y" f }
} "<x></y>" xml-error-test
T{ unclosed f V{ T{ name f "" "x" f } } } "<x>" xml-error-test
T{ nonexist-ns T{ parsing-error f 1 7 } "x" } "<x:y/>" xml-error-test
...
T{ bad-version f "5 million" } "<?xml version='5 million'?><x/>" xml-error-test
