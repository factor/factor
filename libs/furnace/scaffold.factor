...
HELP: crud-word "A CRUD utility function - looks up a word that has been crud-created" ;

: crud-index ( tuple -- )
    dup class dup "crud-index" word-prop crud-word execute ;

: crud-lookup ( string class -- obj )
    get-global [ crud-index = ] subset-with
    dup empty? [ drop f ] [ first ] if ;
HELP: crud-lookup "A CRUD utility function - looks up an object in the store by the pre-designated index." ;

: crud-lookup* ( string class -- tuple )
    tuck crud-lookup
    [ ] [ dup "slot-names" word-prop length 2 + <tuple> ] ?if ;
HELP: crud-lookup* "A CRUD utility function - same as crud-lookup, but always returns a tuple of the given class.  When the lookup fails, returns a tuple of the given class with all slots set to f." ;
...
HELP: scaffold "If realm is not f, then realm is used as the basic authentication realm for the scaffolding actions." ;
