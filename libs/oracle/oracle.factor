...
! Date: 03/27/2007
!
! libs/oracle/oracle.factor
!
! Adapted from oci.h and ociap.h
! Tested with Oracle version - 10.1.0.3 Instant Client

IN: oracle
USING: liboci kernel errors alien namespaces sequences prettyprint math ;
...
C: connection ( username password db -- connection )
    [ set-connection-db ] keep
    [ set-connection-password ] keep
    [ set-connection-username ] keep ;
...
: allocate-error-handle ( -- )
    env get
    "void*" <c-object> dup -rot OCI_HTYPE_ERROR 0 f OCIHandleAlloc 
    check-result *void* err set ;

: allocate-service-handle ( -- )
    env get
    "void*" <c-object> dup -rot OCI_HTYPE_SVCCTX 0 f OCIHandleAlloc 
    check-result *void* svc set ;

: allocate-session-handle ( -- )
    env get
    "void*" <c-object> dup -rot OCI_HTYPE_SESSION 0 f OCIHandleAlloc 
    check-result *void* ses set ;

: allocate-server-handle ( -- )
    env get
    "void*" <c-object> dup -rot OCI_HTYPE_SERVER 0 f OCIHandleAlloc 
...
: allocate-statement-handle ( -- )
    env get
    "void*" <c-object> dup -rot OCI_HTYPE_STMT 0 f OCIHandleAlloc 
